# Popup de tutorial genérico (dash, ataque melee, etc.), basado en inventory_tutorial_popup.gd
# pero con el texto configurable en vez de fijo, para no duplicar toda la lógica de UI por cada tutorial.
extends CanvasLayer
class_name ActionTutorialPopup

const UI_SOUND_PATH = "res://Audio/Sfx/Piano_Ui (2).wav"

var title_text: String = "TUTORIAL"
var desc_text: String = ""
var prompt_text: String = "Presiona [ENTER] o hace click para continuar"
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		if event.physical_keycode == KEY_ENTER or event.physical_keycode == KEY_E:
...
func _create_center_panel(parent: Control) -> void:
	...
	_panel.custom_minimum_size = Vector2(500, 250)
	...
	_panel.pivot_offset = Vector2(250, 125)
...
func _create_content_labels() -> void:
	...
	_desc_lbl.add_theme_color_override("font_color", Color(0.9, 0.9, 0.9))
	_vbox.add_child(_desc_lbl)

	_continue_prompt = Label.new()
	_continue_prompt.text = prompt_text
	...
	_vbox.add_child(_continue_prompt)
...
func _create_continue_button() -> void:
	...
	_continue_btn.pressed.connect(func(): _dismiss_popup())
